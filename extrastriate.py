...
# MAIN MONITOR CYCLE
# =========================================================
def monitor_cycle(active_trades: list, symbol: str = None) -> list:
    """
    Check every active trade once.

    Returns the updated list (closed trades are still in the list
    but marked status="closed").
    """
    ...
    for trade in active_trades:
        ...
        if candle is None:
            # No candle data → keep the trade and try next cycle
            still_active.append(trade)
            continue
...
if __name__ == "__main__":
    ...
    result = monitor_cycle([demo_trade])
    print(f"Active trades after cycle: {len([t for t in result if t['status'] == 'active'])}")
