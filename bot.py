...
def run_h4_mode():
    ...
    if HAS_SPLIT_STATE:
        try:
            ...
            print(f"[H4] save_h1_state failed: {e}")

    # Always try the simple saver
    try:
        save_state(state)
    except Exception as e:
        print(f"[H4] save_state failed: {e}")

    # FORCE write — does not depend on utils at all
    try:
        import json as _json
        os.makedirs("data", exist_ok=True)
        _path = os.path.join("data", "state.json")
        with open(_path, "w") as f:
            _json.dump(state, f, indent=2, default=str)
        print(f"[H4] FORCE saved → {_path} ({os.path.getsize(_path)} bytes)")
    except Exception as e:
        print(f"[H4] FORCE save failed: {e}")
