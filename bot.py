...
try:
    ...
    try:
        ...
        # Absolute minimum fallback so the bot never crashes on missing utils
        def load_state():
            return {
        ...
        def get_active_trades(state, symbol=None):
            ...
            return trades
        def register_trade(state, trade): return state
        def update_active_trades(state, still_active, closed_now): return state
